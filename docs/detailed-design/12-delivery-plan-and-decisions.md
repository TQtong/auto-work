...
> macOS 兼容的五个工作包、ADR-MAC-001～007、风险和发布阶段门见 [macOS 实施计划、风险与 ADR](./macos-compatibility/09-delivery-plan-risks-and-adrs.md)。
...
| ADR | 决策 | 理由 | 后果 |
|---|---|---|---|
| ADR-001 | 单用户本机 loopback Web；支持 Docker/Windows | 个人工作台需要本机仓库，Docker 降低日常启动成本 | 不支持多用户/远程共享；容器只单副本 |
...
| ADR-013 | Docker 使用 AES-256-GCM 持久保险箱并按引用前缀路由 | Linux 无 DPAPI，仍需完整集成功能和认证加密 | 密钥必须备份；DPAPI 迁移需重新配置 |
