...
指引，不能要求应用容器自行控制 Docker daemon 或挂载任意宿主机路径。目录选择按部署能力分级：Windows 本机进程可通过
固定脚本打开原生文件夹窗口，初始路径只经环境变量传入且返回值必须通过绝对路径、真实目录和访问权限验证；Docker/其他平台
只能调用浏览器目录选择器。标准浏览器不暴露绝对路径时只展示目录名并要求人工粘贴，禁止根据目录名推断宿主机位置。
