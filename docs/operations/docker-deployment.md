...
点击“配置并扫描”后，路径输入框右侧的“选择目录”会调用浏览器目录选择器。Chrome/Edge 可以直接打开系统文件夹窗口，
但标准 Web 安全模型通常只向网页返回目录句柄和名称，不返回 `D:/...` 绝对路径；此时界面会保留已选目录名，并要求从
资源管理器地址栏复制完整路径。应用不会根据目录名、当前输入或驱动器做不可靠拼接。Windows 本机进程部署则调用受控的原生
文件夹窗口，可验证并自动回填绝对路径。

也可以直接输入新宿主机路径并复制 `AUTO_WORK_REPOSITORY_PATH="..."`。该输入用于生成安全配置，
